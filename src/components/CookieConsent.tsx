...
const CookieConsent = () => {
  ...
  const save = (choice: Pick<Consent, 'analytics' | 'marketing'>) => {
    ...
    window.dispatchEvent(new Event('stilte-draad:consent-saved'))
    if (consent) setOpen(false)
  }

  if (!open) return null
  return (
    <div ref={dialogRef} className="fixed inset-x-3 bottom-3 z-[70] mx-auto max-h-[calc(100dvh-1.5rem)] max-w-3xl overflow-y-auto rounded-2xl border border-[#c6a978]/60 bg-[#f8f4ec]/95 p-5 shadow-2xl backdrop-blur-md sm:bottom-6 sm:max-h-[calc(100dvh-3rem)] sm:p-6" role="dialog" aria-modal="true" aria-labelledby="cookie-title">
      <div className="flex gap-4"><div aria-hidden="true" className="mt-1 h-10 w-10 shrink-0 rounded-full border border-[#c6a978] bg-[radial-gradient(circle,#c6a978_1px,transparent_1.5px)] [background-size:6px_6px]" /><div><h2 id="cookie-title" className="text-sm font-semibold uppercase tracking-[0.16em]">Ruimte voor jouw keuze</h2><p className="mt-2 text-sm leading-6 text-neutral-600">We gebruiken noodzakelijke browseropslag voor je winkelmand, favorieten en cookievoorkeur. Met jouw toestemming meten we anoniem welke pagina’s worden bezocht, zonder advertentietracking of bezoekersprofielen.</p></div></div>
