...
const SpokenWordPlayer = ({ id }: { id: SpokenWordId }) => {
  ...
  return <div className="mt-6 rounded-xl border border-neutral-800/15 bg-white/25 p-4">
    <audio ref={audioRef} src={track.audioSrc} preload="none" onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onTimeUpdate={(event) => setCurrentTime(event.currentTarget.currentTime)} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onEnded={() => setPlaying(false)} />
