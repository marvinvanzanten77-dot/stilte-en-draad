...
const VerticalNav = ({ zones, activeZone, onSelect, navigate }: Props) => {
  ...
  return <nav className="w-full md:sticky md:top-8 md:h-fit md:w-52 lg:w-60" aria-label="Hoofdnavigatie">
    <div className="flex items-center justify-between md:block">
      ...
      <button ref={menuButtonRef} type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls="mobile-hoofdmenu" className="rounded-full border border-neutral-800/20 bg-white/45 px-4 py-2 text-xs uppercase tracking-[0.16em] md:hidden">{open ? 'Sluiten' : 'Menu'}</button>
      <ul id="mobile-hoofdmenu" className={`${open ? 'flex' : 'hidden'} absolute left-4 right-4 top-24 z-40 flex-col gap-4 rounded-2xl bg-[#f8f4ec]/95 p-6 shadow-soft backdrop-blur-md md:static md:mt-7 md:flex md:bg-transparent md:p-0 md:shadow-none`}>
        ...
        <li className="mt-2 border-t border-neutral-800/10 pt-4 md:hidden"><button type="button" onClick={() => { navigate('/contact'); setOpen(false) }} className="text-xs uppercase tracking-[0.15em] text-neutral-500">Contact</button></li>
        <li className="md:hidden"><button type="button" onClick={() => { navigate('/privacy'); setOpen(false) }} className="text-xs uppercase tracking-[0.15em] text-neutral-500">Privacy</button></li>
