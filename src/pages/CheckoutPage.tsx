        ...
        <form onSubmit={submit} className="grid lg:grid-cols-[1fr_340px]">
          <div className="space-y-8 p-7 md:p-10">
            <fieldset><legend className="text-xs font-semibold uppercase tracking-[0.15em]">Ontvangst</legend>
              <div className={`mt-4 rounded-xl border border-neutral-800/15 p-4 ${pickupReady ? 'bg-white/25' : 'opacity-55'}`}><p className="text-sm font-medium">Afhalen in IJzendoorn</p><p className="mt-2 text-xs leading-5 text-neutral-500">Op afspraak, doorgaans binnen twee tot vijf werkdagen. Verzending is nog niet beschikbaar.</p></div>
            </fieldset>
          ...
          <aside className="border-t border-neutral-800/10 bg-white/20 p-7 lg:border-l lg:border-t-0">
            ...
            <dl className="mt-6 space-y-3 border-t border-neutral-800/10 pt-5 text-sm"><div className="flex justify-between"><dt>Subtotaal</dt><dd>{formatCents(subtotalCents)}</dd></div><div className="flex justify-between"><dt>Afhalen in IJzendoorn</dt><dd>{formatCents(0)}</dd></div><div className="flex justify-between border-t border-neutral-800/10 pt-3 font-semibold"><dt>Totaal</dt><dd>{formatCents(subtotalCents)}</dd></div></dl>
