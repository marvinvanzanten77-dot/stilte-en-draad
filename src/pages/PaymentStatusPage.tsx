...
const PaymentStatusPage = ({ orderId, navigate }: { orderId: string; navigate: (path: string) => void }) => {
  ...
  return <section className="rounded-2xl bg-[#e7ddc9] p-7 text-center shadow-soft md:p-12" aria-live="polite">
    ...
    {paid && <div className="mx-auto mt-7 max-w-lg rounded-xl border border-[#9b7d4f]/25 bg-white/30 p-6 text-left"><p className="text-sm font-medium">Referentie {result.orderNumber}</p>{result.items.length > 0 && <ul className="mt-4 space-y-2 text-sm text-neutral-600">{result.items.map((item) => <li key={item.productId} className="flex justify-between gap-4"><span>{item.title}</span><span>{formatCents(item.unitPriceCents)}</span></li>)}</ul>}<p className="mt-4 border-t border-neutral-800/10 pt-4 text-sm">Totaal: {formatCents(result.totalCents)}</p><p className="mt-4 text-xs leading-5 text-neutral-500">{donation ? 'Je bijdrage gaat naar de aanschaf en verbouwing van de bus.' : result.fulfillment === 'pickup' ? 'We nemen contact op om het ophalen af te spreken.' : 'Je ontvangt vervolginformatie over de verzending.'}</p></div>}
