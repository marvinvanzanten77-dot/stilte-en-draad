...
type OrderResult = {
  ...
  address: { line: string; postalCode: string; city: string; country: string } | null
...
const PaymentStatusPage = ({ orderId, navigate }: { orderId: string; navigate: (path: string) => void }) => {
  ...
  return <section className="rounded-2xl bg-[#e7ddc9] p-7 text-center shadow-soft md:p-12" aria-live="polite">
    ...
    {paid && <div className="mx-auto mt-7 max-w-lg rounded-xl border border-[#9b7d4f]/25 bg-white/30 p-6 text-left"><p className="text-sm font-medium">Referentie {result.orderNumber}</p>{result.items.length > 0 && <ul className="mt-4 space-y-2 text-sm text-neutral-600">{result.items.map((item) => <li key={item.productId} className="flex justify-between gap-4"><span>{item.title}</span><span>{formatCents(item.unitPriceCents)}</span></li>)}</ul>}{!donation && <dl className="mt-4 space-y-2 border-t border-neutral-800/10 pt-4 text-sm"><div className="flex justify-between"><dt>Subtotaal</dt><dd>{formatCents(result.subtotalCents)}</dd></div><div className="flex justify-between"><dt>{result.fulfillment === 'shipping' ? 'Verzendkosten' : 'Afhalen'}</dt><dd>{formatCents(result.shippingCents)}</dd></div></dl>}<p className="mt-4 border-t border-neutral-800/10 pt-4 text-sm">Totaal: {formatCents(result.totalCents)}</p><p className="mt-4 text-xs leading-5 text-neutral-500">{donation ? 'Je bijdrage gaat naar de aanschaf en verbouwing van de bus.' : result.fulfillment === 'pickup' ? 'Afhalen op afspraak in IJzendoorn, doorgaans binnen twee tot vijf werkdagen. We nemen persoonlijk contact met je op om een geschikt moment af te spreken.' : 'Verzenden binnen Nederland.'}</p>{!donation && result.fulfillment === 'shipping' && result.address && <address className="mt-3 text-xs not-italic leading-5 text-neutral-600">{result.address.line}<br />{result.address.postalCode} {result.address.city}<br />Nederland</address>}</div>}
