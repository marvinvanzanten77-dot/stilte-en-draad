import { productImage, productThumbnail, products, type Product } from '../data/products'
...
const ProductPage = ({ product, navigate, certificate = false }: ProductPageProps) => {
  ...
  return (
    <div className="space-y-7">
      <article className="overflow-hidden rounded-2xl bg-[#e7ddc9] shadow-soft"><div className="grid xl:grid-cols-2">
        <div className="flex min-h-0 flex-col overflow-hidden">
          <div className="relative min-h-0 flex-1 overflow-hidden">
            <img src={productImage(product)} srcSet={`${productThumbnail(product)} 540w, ${productImage(product)} 864w`} sizes="(min-width: 1280px) 50vw, 100vw" width="864" height="1152" fetchPriority="high" alt={`${product.title}, handgemaakt ${product.category.toLowerCase()} door Jannie van Zanten`} className="h-full max-h-[760px] w-full object-cover" />
      ...
      <section><h2 className="text-xs uppercase tracking-[0.18em] text-neutral-500">Verwante werken</h2><div className="mt-4 grid gap-4 sm:grid-cols-3">{related.map((item) => <button key={item.id} type="button" onClick={() => navigate(`/werk/${item.slug}`)} className="overflow-hidden rounded-xl bg-white/35 text-left"><img src={productThumbnail(item)} width="540" height="720" alt={`${item.title}, verwant handgemaakt werk`} loading="lazy" className="aspect-square w-full object-cover" /><span className="block p-4 text-xs uppercase tracking-[0.13em]">{item.title}</span></button>)}</div></section>
      ...
      {!certificate && <ProductShare product={product} />}
    </div>
  )
}
